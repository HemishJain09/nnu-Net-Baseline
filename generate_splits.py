...
def generate_splits(nnunet_raw: Path, nnunet_preprocessed: Path, marksheet_path: Path, train_centers: list, bootcamp: bool = False, test_mode: bool = False):
    case_ids = []
    ...
    if test_mode:
        print(f"\n🚨 TEST MODE ACTIVE: Creating custom evaluation splits for PCNN! 🚨")
        pcnn_cases = [c for c in case_ids if patient_to_center.get(c.split("_")[0], "UNKNOWN") == "PCNN"]
        dummy_train = [c for c in case_ids if patient_to_center.get(c.split("_")[0], "UNKNOWN") != "PCNN"][:5]
        
        splits = []
        for i in range(NUM_FOLDS):
            splits.append({
                "train": sorted(dummy_train),
                "val": sorted(pcnn_cases),
            })
        ...
        print(f"Generated test splits with {len(pcnn_cases)} PCNN validation cases.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_mode", action="store_true", help="Test mode: Put PCNN holdout cases into the validation split")
    args = parser.parse_args()
    
    generate_splits(Path(args.nnunet_raw), Path(args.nnunet_preprocessed), Path(args.marksheet), args.train_centers, args.bootcamp, args.test_mode)
